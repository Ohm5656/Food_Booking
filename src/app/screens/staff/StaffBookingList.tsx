  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      <div className="max-w-4xl mx-auto pb-24 md:pb-8">
        {/* Header */}
        <header className="bg-white px-5 pt-safe pt-4 pb-3 border-b border-slate-100 sticky top-0 z-40 shadow-[0_4px_10px_-10px_rgba(0,0,0,0.1)] md:rounded-b-3xl">
          <h1 className="text-xl md:text-2xl font-bold text-slate-800 mb-4 tracking-tight">การจองทั้งหมด</h1>
          ...
          <div className="flex space-x-3 mb-4">
            <div className="relative flex-1">
              ...
              <input 
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 md:py-3.5 pl-10 md:pl-12 pr-4 text-sm md:text-base focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all placeholder:font-light"
              />
            </div>
            <button 
              onClick={() => setShowFilter(true)}
              className="bg-white border border-slate-200 rounded-xl p-2.5 md:p-3.5 text-slate-600 hover:bg-slate-50 transition-colors shadow-sm"
            >
          ...
          <div className="flex overflow-x-auto space-x-2 pb-1 scrollbar-hide">
            {filters.map(f => (
              <button
                ...
                className={clsx(
                  "flex-none px-4 py-1.5 md:px-6 md:py-2 rounded-full text-xs md:text-sm font-medium border transition-all",
                  activeFilter === f.id 
        ...
        {/* List */}
        <main className="p-4 space-y-3 md:space-y-4">
        {bookings.map((b, i) => (
          <motion.div 
            key={b.id}
        ...
        {showFilter && (
          <>
            ...
            <motion.div 
              ...
              className="fixed bottom-0 left-0 right-0 md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:bottom-auto md:w-full md:max-w-md bg-white rounded-t-[32px] md:rounded-3xl p-6 z-50 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)] pb-safe md:pb-6"
            >
